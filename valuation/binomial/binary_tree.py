...
class BinaryTree():
    ...
    def isLeaf(self):
        return self.getDownChild() is None and self.getUpChild() is None
...
def plot(tree):
    ...
    if not tree.isLeaf():
        ...
        if tree.getDownChild() is None and tree.getUpChild() is None:
...
#    option = Option(S0, strike, maturity, r, volatility, n, style, type, asset)
    ...
    for p in noeuds:
        if p[3] is True:
            ax.annotate(str(round(p[1],2))+"\n\nPayOff="+str(round(p[2], 2)), xy=p[0],
                        arrowprops=dict(facecolor='black'), label="ss")
        # print(p[0])
        else:
